...
def fieldplot(fig, ax, x, m, WL, comment='', WL_units=' ', crossplane='XZ',
              ...
              is_flow_extend=True, pl=-1, outline_width=2, subplot_label=' '):
    ...
    try:
        ...
        ax.xaxis.set_tick_params(width=outline_width)
        ax.yaxis.set_tick_params(width=outline_width)
        ...
        for xx in [x[0],x[-1]]:
            r = xx * WL / 2.0 / np.pi
            s1 = patches.Arc((0, 0), 2.0 * r, 2.0 * r,  angle=0.0, zorder=1.98,
        ...
        if (not crossplane == 'XY') and flow_total > 0:
            ...
            for flow in rg:
                ...
                patch = patches.PathPatch(
                    path, facecolor='none', lw=outline_width/2, edgecolor='white', zorder=1.9, alpha=0.7)
                # patch = patches.PathPatch(
