...
def GetCSTField(cst_data_txt, WL):
    x=np.transpose(np.loadtxt(cst_data_txt))
    E = np.sqrt(np.absolute(x[3]+1.0j*x[6])**2+np.absolute(x[4]+1.0j*x[7])**2+np.absolute(x[5]+1.0j*x[8])**2)
    coordX = np.unique(x[1])/WL
    coordZ = np.unique(x[2])/WL
    print(x[2])
